...
let emailText = `
  ...
  <head>
    ...
    <style>
      ...
      .email-container {
        background-color: #02B8F5; /* Pink background for the entire email */
...
if (productInfo.some(product => getDiscountList(product.product_id))) {
  ...
  emailText += `<p class="email-text">All Items in your cart:</p>`;
}
...
emailText += `
        ...
        <p style="text-align: center;">
          <a class="button" href="http://localhost:8000/men" style="background-color: #02B8F5; color: #FFFFFF; padding: 10px 20px; text-decoration: none; border-radius: 5px; display: inline-block; border: 2px solid #FFFFFF; margin: 0 auto;">Shop Now</a>
        </p>
      </div>
    </div>
  </body>
...
let mailOptions = {
  ...
  subject: 'Your Cart misses you', // Subject line
